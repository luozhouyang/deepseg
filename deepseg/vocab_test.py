...
class VocabTest(tf.test.TestCase):
    ...
    def testGenerateLabelVocab(self):
        ...
        vocab.generate_label_vocab(feat_file, feat_vocab_file)

    def testCreateVocabTable(self):
        feat_vocab_file = "testdata/vocab.feature.txt"
        word2idx, idx2word = vocab.create_features_vocab_table(feat_vocab_file)
        with self.test_session() as sess:
            sess.run(tf.tables_initializer())
            idx = word2idx.lookup(tf.constant(vocab.UNK_CHARS, dtype=tf.string))
            self.assertEqual(0, sess.run(idx))
            word = idx2word.lookup(tf.constant(vocab.UNK_CHARS_ID, dtype=tf.int64))
            self.assertEqual(vocab.UNK_CHARS, sess.run(word).decode("utf8"))
